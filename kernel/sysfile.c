...
int accesscheck(struct inode *ip, uint mode, char *path)
{
  ...
  if (owner == uid)
  {
    if (mode == O_RDONLY && (ip->permissions & 0400))
      return 1;
    if (mode == O_WRONLY && (ip->permissions & 0200))
      return 1;
    if (mode == O_RDWR && (ip->permissions & 0600))
      return 1;
  }
  else if (gid == getgid(owner))
  {
    if (mode == O_RDONLY && (ip->permissions & 0040))
      return 1;
    if (mode == O_WRONLY && (ip->permissions & 0020))
      return 1;
    if (mode == O_RDWR && (ip->permissions & 0060))
      return 1;
  }
  else
  {
    if (mode == O_RDONLY && (ip->permissions & 0004))
      return 1;
    if (mode == O_WRONLY && (ip->permissions & 0002))
      return 1;
    if (mode == O_RDWR && (ip->permissions & 0006))
      return 1;
  }
