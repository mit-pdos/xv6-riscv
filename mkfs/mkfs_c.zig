...
pub const struct_xv6_stat = extern struct {
    dev: c_int,
    ino: uint,
    type: c_short,
    nlink: c_short,
...
pub export var nbitmap: c_int = @divTrunc(@as(c_int, 2000), @as(c_int, 1024) * @as(c_int, 8)) + @as(c_int, 1);
pub export var ninodeblocks: c_int = @bitCast(c_int, @truncate(c_uint, (@bitCast(c_ulong, @as(c_long, @as(c_int, 200))) / (@bitCast(c_ulong, @as(c_long, @as(c_int, 1024))) / @sizeOf(struct_dinode))) +% @bitCast(c_ulong, @as(c_long, @as(c_int, 1)))));
pub export var nlog: c_int = @as(c_int, 10) * @as(c_int, 3);
pub export var nmeta: c_int = @import("std").mem.zeroes(c_int);
pub export var nblocks: c_int = @import("std").mem.zeroes(c_int);
pub export var fsfd: c_int = @import("std").mem.zeroes(c_int);
pub export var sb: struct_superblock = @import("std").mem.zeroes(struct_superblock);
pub export var zeroes: [1024]u8 = @import("std").mem.zeroes([1024]u8);
pub export var freeinode: uint = 1;
pub export var freeblock: uint = @import("std").mem.zeroes(uint);
pub export fn balloc(arg_used: c_int) void {
    var used = arg_used;
    var buf: [1024]uchar = undefined;
    var i: c_int = undefined;
    _ = blk: {
        _ = @sizeOf(c_int);
        break :blk blk_1: {
            break :blk_1 if (used < (@as(c_int, 1024) * @as(c_int, 8))) {} else {
                __assert_fail("used < BSIZE*8", "mkfs/mkfs.c", @bitCast(c_uint, @as(c_int, 241)), "void balloc(int)");
            };
        };
    };
    bzero(@ptrCast(?*anyopaque, @ptrCast([*c]uchar, @alignCast(@import("std").meta.alignment([*c]uchar), &buf))), @bitCast(c_ulong, @as(c_long, @as(c_int, 1024))));
    {
        i = 0;
        while (i < used) : (i += 1) {
            buf[@intCast(c_uint, @divTrunc(i, @as(c_int, 8)))] = @bitCast(uchar, @truncate(i8, @bitCast(c_int, @as(c_uint, buf[@intCast(c_uint, @divTrunc(i, @as(c_int, 8)))])) | (@as(c_int, 1) << @intCast(@import("std").math.Log2Int(c_int), @import("std").zig.c_translation.signedRemainder(i, @as(c_int, 8))))));
        }
    }
    wsect(sb.bmapstart, @ptrCast(?*anyopaque, @ptrCast([*c]uchar, @alignCast(@import("std").meta.alignment([*c]uchar), &buf))));
}
pub export fn wsect(arg_sec: uint, arg_buf: ?*anyopaque) void {
    var sec = arg_sec;
    var buf = arg_buf;
    if (lseek(fsfd, @bitCast(__off_t, @as(c_ulong, sec *% @bitCast(c_uint, @as(c_int, 1024)))), @as(c_int, 0)) != @bitCast(c_long, @as(c_ulong, sec *% @bitCast(c_uint, @as(c_int, 1024))))) {
        die("lseek");
    }
    if (write(fsfd, buf, @bitCast(usize, @as(c_long, @as(c_int, 1024)))) != @bitCast(c_long, @as(c_long, @as(c_int, 1024)))) {
        die("write");
    }
}
pub export fn winode(arg_inum: uint, arg_ip: [*c]struct_dinode) void {
    var inum = arg_inum;
    var ip = arg_ip;
    var buf: [1024]u8 = undefined;
    var bn: uint = undefined;
    var dip: [*c]struct_dinode = undefined;
    bn = @bitCast(uint, @truncate(c_uint, (@bitCast(c_ulong, @as(c_ulong, inum)) / (@bitCast(c_ulong, @as(c_long, @as(c_int, 1024))) / @sizeOf(struct_dinode))) +% @bitCast(c_ulong, @as(c_ulong, sb.inodestart))));
    rsect(bn, @ptrCast(?*anyopaque, @ptrCast([*c]u8, @alignCast(@import("std").meta.alignment([*c]u8), &buf))));
    dip = @ptrCast([*c]struct_dinode, @alignCast(@import("std").meta.alignment([*c]struct_dinode), @ptrCast([*c]u8, @alignCast(@import("std").meta.alignment([*c]u8), &buf)))) + (@bitCast(c_ulong, @as(c_ulong, inum)) % (@bitCast(c_ulong, @as(c_long, @as(c_int, 1024))) / @sizeOf(struct_dinode)));
    dip.* = ip.*;
    wsect(bn, @ptrCast(?*anyopaque, @ptrCast([*c]u8, @alignCast(@import("std").meta.alignment([*c]u8), &buf))));
}
pub export fn rinode(arg_inum: uint, arg_ip: [*c]struct_dinode) void {
    var inum = arg_inum;
    var ip = arg_ip;
    var buf: [1024]u8 = undefined;
    var bn: uint = undefined;
...
pub export fn rsect(arg_sec: uint, arg_buf: ?*anyopaque) void {
    var sec = arg_sec;
    var buf = arg_buf;
    if (lseek(fsfd, @bitCast(__off_t, @as(c_ulong, sec *% @bitCast(c_uint, @as(c_int, 1024)))), @as(c_int, 0)) != @bitCast(c_long, @as(c_ulong, sec *% @bitCast(c_uint, @as(c_int, 1024))))) {
        die("lseek");
    }
    if (read(fsfd, buf, @bitCast(usize, @as(c_long, @as(c_int, 1024)))) != @bitCast(c_long, @as(c_long, @as(c_int, 1024)))) {
        die("read");
    }
}
pub export fn ialloc(arg_type: ushort) uint {
    var @"type" = arg_type;
    var inum: uint = blk: {
        const ref = &freeinode;
        const tmp = ref.*;
    ...
    var din: struct_dinode = undefined;
    bzero(@ptrCast(?*anyopaque, &din), @sizeOf(struct_dinode));
    din.type = @bitCast(c_short, xshort(@"type"));
    din.nlink = @bitCast(c_short, xshort(@bitCast(ushort, @truncate(c_short, @as(c_int, 1)))));
    din.size = xint(@bitCast(uint, @as(c_int, 0)));
    winode(inum, &din);
    return inum;
}
pub export fn iappend(arg_inum: uint, arg_xp: ?*anyopaque, arg_n: c_int) void {
    var inum = arg_inum;
    var xp = arg_xp;
    var n = arg_n;
    var p: [*c]u8 = @ptrCast([*c]u8, @alignCast(@import("std").meta.alignment([*c]u8), xp));
    var fbn: uint = undefined;
    var off: uint = undefined;
    var n1: uint = undefined;
    var din: struct_dinode = undefined;
    var buf: [1024]u8 = undefined;
    var indirect: [256]uint = undefined;
    var x: uint = undefined;
    rinode(inum, &din);
    off = xint(din.size);

    while (n > @as(c_int, 0)) {
        fbn = off / @bitCast(c_uint, @as(c_int, 1024));
        _ = blk: {
            _ = @sizeOf(c_int);
            break :blk blk_1: {
                break :blk_1 if (@bitCast(c_ulong, @as(c_ulong, fbn)) < (@bitCast(c_ulong, @as(c_long, @as(c_int, 12))) +% (@bitCast(c_ulong, @as(c_long, @as(c_int, 1024))) / @sizeOf(uint)))) {} else {
                    __assert_fail("fbn < MAXFILE", "mkfs/mkfs.c", @bitCast(c_uint, @as(c_int, 267)), "void iappend(uint, void *, int)");
                };
            };
        };
        if (fbn < @bitCast(c_uint, @as(c_int, 12))) {
            if (xint(din.addrs[fbn]) == @bitCast(c_uint, @as(c_int, 0))) {
                din.addrs[fbn] = xint(blk: {
                    const ref = &freeblock;
                    const tmp = ref.*;
                    ref.* +%= 1;
                    break :blk tmp;
            ...
            if (xint(din.addrs[@intCast(c_uint, @as(c_int, 12))]) == @bitCast(c_uint, @as(c_int, 0))) {
                din.addrs[@intCast(c_uint, @as(c_int, 12))] = xint(blk: {
                    const ref = &freeblock;
                    const tmp = ref.*;
                    ref.* +%= 1;
                    break :blk tmp;
            ...
            rsect(xint(din.addrs[@intCast(c_uint, @as(c_int, 12))]), @ptrCast(?*anyopaque, @ptrCast([*c]u8, @alignCast(@import("std").meta.alignment([*c]u8), @ptrCast([*c]uint, @alignCast(@import("std").meta.alignment([*c]uint), &indirect))))));
            if (indirect[fbn -% @bitCast(c_uint, @as(c_int, 12))] == @bitCast(c_uint, @as(c_int, 0))) {
                indirect[fbn -% @bitCast(c_uint, @as(c_int, 12))] = xint(blk: {
                    const ref = &freeblock;
                    const tmp = ref.*;
                    ref.* +%= 1;
                    break :blk tmp;
                });
                wsect(xint(din.addrs[@intCast(c_uint, @as(c_int, 12))]), @ptrCast(?*anyopaque, @ptrCast([*c]u8, @alignCast(@import("std").meta.alignment([*c]u8), @ptrCast([*c]uint, @alignCast(@import("std").meta.alignment([*c]uint), &indirect))))));
            }
            x = xint(indirect[fbn -% @bitCast(c_uint, @as(c_int, 12))]);
        }
        n1 = if (@bitCast(c_uint, n) < (((fbn +% @bitCast(c_uint, @as(c_int, 1))) *% @bitCast(c_uint, @as(c_int, 1024))) -% off)) @bitCast(c_uint, n) else ((fbn +% @bitCast(c_uint, @as(c_int, 1))) *% @bitCast(c_uint, @as(c_int, 1024))) -% off;
        rsect(x, @ptrCast(?*anyopaque, @ptrCast([*c]u8, @alignCast(@import("std").meta.alignment([*c]u8), &buf))));
        bcopy(@ptrCast(?*const anyopaque, p), @ptrCast(?*anyopaque, (@ptrCast([*c]u8, @alignCast(@import("std").meta.alignment([*c]u8), &buf)) + off) - (fbn *% @bitCast(c_uint, @as(c_int, 1024)))), @bitCast(usize, @as(c_ulong, n1)));
        wsect(x, @ptrCast(?*anyopaque, @ptrCast([*c]u8, @alignCast(@import("std").meta.alignment([*c]u8), &buf))));
        n -= @bitCast(c_int, n1);
        off +%= n1;
        p += n1;
    }
...
pub export fn die(arg_s: [*c]const u8) void {
    var s = arg_s;
    perror(s);
    exit(@as(c_int, 1));
}
pub export fn xshort(arg_x: ushort) ushort {
    var x = arg_x;
    var y: ushort = undefined;
    var a: [*c]uchar = @ptrCast([*c]uchar, @alignCast(@import("std").meta.alignment([*c]uchar), &y));
    a[@intCast(c_uint, @as(c_int, 0))] = @bitCast(uchar, @truncate(u8, x));
    a[@intCast(c_uint, @as(c_int, 1))] = @bitCast(uchar, @truncate(i8, @bitCast(c_int, @as(c_uint, x)) >> @intCast(@import("std").math.Log2Int(c_int), 8)));
    return y;
}
pub export fn xint(arg_x: uint) uint {
    var x = arg_x;
    var y: uint = undefined;
    var a: [*c]uchar = @ptrCast([*c]uchar, @alignCast(@import("std").meta.alignment([*c]uchar), &y));
    a[@intCast(c_uint, @as(c_int, 0))] = @bitCast(uchar, @truncate(u8, x));
    a[@intCast(c_uint, @as(c_int, 1))] = @bitCast(uchar, @truncate(u8, x >> @intCast(@import("std").math.Log2Int(uint), 8)));
    a[@intCast(c_uint, @as(c_int, 2))] = @bitCast(uchar, @truncate(u8, x >> @intCast(@import("std").math.Log2Int(uint), 16)));
    a[@intCast(c_uint, @as(c_int, 3))] = @bitCast(uchar, @truncate(u8, x >> @intCast(@import("std").math.Log2Int(uint), 24)));
    return y;
} // /usr/include/assert.h:140:24: warning: ignoring StaticAssert declaration
pub export fn main(arg_argc: c_int, arg_argv: [*c][*c]u8) c_int {
    ...
    var rootino: uint = undefined;
    var inum: uint = undefined;
    var off: uint = undefined;
    var de: struct_dirent = undefined;
    var buf: [1024]u8 = undefined;
    var din: struct_dinode = undefined;
    if (argc < @as(c_int, 2)) {
        _ = fprintf(stderr, "Usage: mkfs fs.img files...\n");
        exit(@as(c_int, 1));
    }
    _ = blk: {
        _ = @sizeOf(c_int);
        break :blk blk_1: {
            break :blk_1 if ((@bitCast(c_ulong, @as(c_long, @as(c_int, 1024))) % @sizeOf(struct_dinode)) == @bitCast(c_ulong, @as(c_long, @as(c_int, 0)))) {} else {
                __assert_fail("(BSIZE % sizeof(struct dinode)) == 0", "mkfs/mkfs.c", @bitCast(c_uint, @as(c_int, 85)), "int main(int, char **)");
            };
        };
    };
    _ = blk: {
        _ = @sizeOf(c_int);
        break :blk blk_1: {
            break :blk_1 if ((@bitCast(c_ulong, @as(c_long, @as(c_int, 1024))) % @sizeOf(struct_dirent)) == @bitCast(c_ulong, @as(c_long, @as(c_int, 0)))) {} else {
                __assert_fail("(BSIZE % sizeof(struct dirent)) == 0", "mkfs/mkfs.c", @bitCast(c_uint, @as(c_int, 86)), "int main(int, char **)");
            };
        };
    };
    fsfd = open(argv[@intCast(c_uint, @as(c_int, 1))], (@as(c_int, 2) | @as(c_int, 64)) | @as(c_int, 512), @as(c_int, 438));
    if (fsfd < @as(c_int, 0)) {
        die(argv[@intCast(c_uint, @as(c_int, 1))]);
    }
    nmeta = ((@as(c_int, 2) + nlog) + ninodeblocks) + nbitmap;
    nblocks = @as(c_int, 2000) - nmeta;
    sb.magic = @bitCast(uint, @as(c_int, 270544960));
    sb.size = xint(@bitCast(uint, @as(c_int, 2000)));
    sb.nblocks = xint(@bitCast(uint, nblocks));
    sb.ninodes = xint(@bitCast(uint, @as(c_int, 200)));
    sb.nlog = xint(@bitCast(uint, nlog));
    sb.logstart = xint(@bitCast(uint, @as(c_int, 2)));
    sb.inodestart = xint(@bitCast(uint, @as(c_int, 2) + nlog));
    sb.bmapstart = xint(@bitCast(uint, (@as(c_int, 2) + nlog) + ninodeblocks));
    freeblock = @bitCast(uint, nmeta);
    {
        i = 0;
        while (i < @as(c_int, 2000)) : (i += 1) {
            wsect(@bitCast(uint, i), @ptrCast(?*anyopaque, @ptrCast([*c]u8, @alignCast(@import("std").meta.alignment([*c]u8), &zeroes))));
        }
    }
    _ = memset(@ptrCast(?*anyopaque, @ptrCast([*c]u8, @alignCast(@import("std").meta.alignment([*c]u8), &buf))), @as(c_int, 0), @sizeOf([1024]u8));
    _ = memmove(@ptrCast(?*anyopaque, @ptrCast([*c]u8, @alignCast(@import("std").meta.alignment([*c]u8), &buf))), @ptrCast(?*const anyopaque, &sb), @sizeOf(struct_superblock));
    wsect(@bitCast(uint, @as(c_int, 1)), @ptrCast(?*anyopaque, @ptrCast([*c]u8, @alignCast(@import("std").meta.alignment([*c]u8), &buf))));
    rootino = ialloc(@bitCast(ushort, @truncate(c_short, @as(c_int, 1))));
    _ = blk: {
        _ = @sizeOf(c_int);
        break :blk blk_1: {
            break :blk_1 if (rootino == @bitCast(c_uint, @as(c_int, 1))) {} else {
                __assert_fail("rootino == ROOTINO", "mkfs/mkfs.c", @bitCast(c_uint, @as(c_int, 118)), "int main(int, char **)");
            };
        };
    };
    bzero(@ptrCast(?*anyopaque, &de), @sizeOf(struct_dirent));
    de.inum = xshort(@bitCast(ushort, @truncate(c_ushort, rootino)));
    _ = strcpy(@ptrCast([*c]u8, @alignCast(@import("std").meta.alignment([*c]u8), &de.name)), ".");
    iappend(rootino, @ptrCast(?*anyopaque, &de), @bitCast(c_int, @truncate(c_uint, @sizeOf(struct_dirent))));
    bzero(@ptrCast(?*anyopaque, &de), @sizeOf(struct_dirent));
    de.inum = xshort(@bitCast(ushort, @truncate(c_ushort, rootino)));
    _ = strcpy(@ptrCast([*c]u8, @alignCast(@import("std").meta.alignment([*c]u8), &de.name)), "..");
    iappend(rootino, @ptrCast(?*anyopaque, &de), @bitCast(c_int, @truncate(c_uint, @sizeOf(struct_dirent))));
    {
        i = 2;
        while (i < argc) : (i += 1) {
            var shortname: [*c]u8 = undefined;
            if (strncmp((blk: {
                const tmp = i;
                if (tmp >= 0) break :blk argv + @intCast(usize, tmp) else break :blk argv - ~@bitCast(usize, @intCast(isize, tmp) +% -1);
            }).*, "zig-out/bin/", @bitCast(c_ulong, @as(c_long, @as(c_int, 12)))) == @as(c_int, 0)) {
                shortname = (blk: {
                    const tmp = i;
                    if (tmp >= 0) break :blk argv + @intCast(usize, tmp) else break :blk argv - ~@bitCast(usize, @intCast(isize, tmp) +% -1);
                }).* + @bitCast(usize, @intCast(isize, @as(c_int, 12)));
            } else {
                shortname = (blk: {
                    const tmp = i;
                    if (tmp >= 0) break :blk argv + @intCast(usize, tmp) else break :blk argv - ~@bitCast(usize, @intCast(isize, tmp) +% -1);
            ...
            _ = blk: {
                _ = @sizeOf(c_int);
                break :blk blk_1: {
                    break :blk_1 if (index(shortname, @as(c_int, '/')) == null) {} else {
                        __assert_fail("index(shortname, '/') == 0", "mkfs/mkfs.c", @bitCast(c_uint, @as(c_int, 138)), "int main(int, char **)");
                    };
                };
            };
            if ((blk: {
                const tmp = open((blk_1: {
                    const tmp_2 = i;
                    if (tmp_2 >= 0) break :blk_1 argv + @intCast(usize, tmp_2) else break :blk_1 argv - ~@bitCast(usize, @intCast(isize, tmp_2) +% -1);
                }).*, @as(c_int, 0));
                fd = tmp;
                break :blk tmp;
            }) < @as(c_int, 0)) {
            ...
            if (@bitCast(c_int, @as(c_uint, shortname[@intCast(c_uint, @as(c_int, 0))])) == @as(c_int, '_')) {
                shortname += @bitCast(usize, @intCast(isize, @as(c_int, 1)));
            }
            inum = ialloc(@bitCast(ushort, @truncate(c_short, @as(c_int, 2))));
            bzero(@ptrCast(?*anyopaque, &de), @sizeOf(struct_dirent));
            de.inum = xshort(@bitCast(ushort, @truncate(c_ushort, inum)));
            _ = strncpy(@ptrCast([*c]u8, @alignCast(@import("std").meta.alignment([*c]u8), &de.name)), shortname, @bitCast(c_ulong, @as(c_long, @as(c_int, 14))));
            iappend(rootino, @ptrCast(?*anyopaque, &de), @bitCast(c_int, @truncate(c_uint, @sizeOf(struct_dirent))));
            while ((blk: {
                const tmp = @bitCast(c_int, @truncate(c_int, read(fd, @ptrCast(?*anyopaque, @ptrCast([*c]u8, @alignCast(@import("std").meta.alignment([*c]u8), &buf))), @sizeOf([1024]u8))));
                cc = tmp;
                break :blk tmp;
            }) > @as(c_int, 0)) {
                iappend(inum, @ptrCast(?*anyopaque, @ptrCast([*c]u8, @alignCast(@import("std").meta.alignment([*c]u8), &buf))), cc);
            }
            _ = close(fd);
        }
    }
    rinode(rootino, &din);
    off = xint(din.size);
    off = ((off / @bitCast(c_uint, @as(c_int, 1024))) +% @bitCast(c_uint, @as(c_int, 1))) *% @bitCast(c_uint, @as(c_int, 1024));
    din.size = xint(off);
    winode(rootino, &din);
    balloc(@bitCast(c_int, freeblock));
    exit(@as(c_int, 0));
    return 0;
}
...
pub const stat = xv6_stat;
pub const ROOTINO = @as(c_int, 1);
pub const BSIZE = @as(c_int, 1024);
pub const FSMAGIC = @import("std").zig.c_translation.promoteIntLiteral(c_int, 0x10203040, .hexadecimal);
...
pub inline fn IBLOCK(i: anytype, sb_1: anytype) @TypeOf(@import("std").zig.c_translation.MacroArithmetic.div(i, IPB) + sb_1.inodestart) {
    return @import("std").zig.c_translation.MacroArithmetic.div(i, IPB) + sb_1.inodestart;
}
pub const BPB = BSIZE * @as(c_int, 8);
pub inline fn BBLOCK(b: anytype, sb_1: anytype) @TypeOf(@import("std").zig.c_translation.MacroArithmetic.div(b, BPB) + sb_1.bmapstart) {
    return @import("std").zig.c_translation.MacroArithmetic.div(b, BPB) + sb_1.bmapstart;
}
...
pub const NINODES = @as(c_int, 200);
...
pub const xv6_stat = struct_xv6_stat;
